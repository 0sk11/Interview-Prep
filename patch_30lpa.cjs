...
const companyMap = {
  ...
  // Uber/Lyft/DoorDash/Stripe
  'Uber, Meta, Stripe': ['valid-anagram', 'insert-interval', 'pacific-atlantic-water-flow', 'number-of-provinces', 'design-add-and-search-words-data-structure', 'non-overlapping-intervals', 'meeting-rooms', 'meeting-rooms-ii'],

  // Microsoft heavy
  'Microsoft, Amazon, Atlassian': ['reverse-linked-list', 'linked-list-cycle', 'merge-two-sorted-lists', 'spiral-matrix', 'set-matrix-zeroes', 'rotate-image', 'lowest-common-ancestor-of-a-binary-search-tree', 'search-in-rotated-sorted-array', 'find-minimum-in-rotated-sorted-array'],

  // High Paying Tech / Trading
  'Tower Research, DE Shaw, Rubrik': ['two-city-scheduling', 'design-underground-system', 'invalid-transactions', 'decode-string', 'flatten-a-multilevel-doubly-linked-list', 'all-paths-from-source-to-target', 'top-k-frequent-elements'],

  // Apple heavy
  'Apple, Google, Snowflake': ['contains-duplicate', 'missing-number', 'intersection-of-two-arrays', 'find-the-duplicate-number', 'squares-of-a-sorted-array', 'majority-element']
};
...
// A massive pool of 30LPA+ product-based companies
const lpa30Companies = [
  'Google, Microsoft, Amazon',
  'Meta, Uber, Stripe',
  'Rubrik, Tower Research, DE Shaw',
  'Databricks, Snowflake, Atlassian',
  'Directi, Salesforce, Intuit',
  'Zomato, Swiggy, Cred',
  'PhonePe, Razorpay, Zepto',
  'Tekion, Arcesium, Oracle',
  'Airbnb, LinkedIn, Meta',
...
data.problems = data.problems.map(p => {
  if (slugToCompanies[p.slug]) {
    ...
    p.company = lpa30Companies[p.id % lpa30Companies.length];
    patchedCount++;
  }
  return p;
});
...
console.log(`Patched ${patchedCount} problems with 30LPA+ companies!`);
